...
function work(valBytes, time, difficulty, button) {
    const timeBytes = new Uint8Array(8)
    const dv = new DataView(timeBytes.buffer)
    dv.setBigUint64(0, BigInt(time), true)
    const load = new Uint8Array(valBytes.length + timeBytes.length)
    load.set(valBytes)
    load.set(timeBytes, valBytes.length)
    const loadhash = blake2b(load)
    const salt = new Uint8Array(32)
    const input = new Uint8Array(loadhash.length + salt.length)
    let i = 0
...
// Thank you dcposch. Modified by Joey Innes.
